    ...
    try:
        with Session(engine) as session:
            ...
            if (
                ...
                session.add(biz)
                session.commit()
            result = sheets_mod.sync_products(
                company_name, products, spreadsheet_id=sheet_id, session=session, user=owner
            )
...
async def extract_products_from_url(req: UrlParseRequest, _user: AuthUser = Depends(get_current_user)):
    ...
    # Prefer structured scrape; AI only fills gaps when the HTML crawl is thin
    ...
    if merged:
        ...
        message = (
            "No products were found on that page. Try the shop homepage or a "
            "/product-category/ URL, upload a catalog file, or add items manually."
        )
    return {
        "sourceUrl": req.url,
